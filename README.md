...
| 链路 | `enter_match.json` v7 状态机：**任意界面启动/中断恢复都自动接续**——模式选择→点烽火地带→选零号大坝→开始行动→确认配装→入局提醒→出发→匹配→干员选择；点击后 `expect_screen` 轮询验证、失败软跳转+人工确认；未知界面自动 `mark_unknown`（截图+OCR 词表留证）；结算画面自动空格跳过；**"空格继续"类弹层（仓库升级完成/广告/任务领取等）自动按键关闭**，不再卡链路 |
